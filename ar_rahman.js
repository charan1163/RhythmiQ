// Fetch and render for A. R. Rahman songs from iTunes

async function fetchArSongs() {
    const url = `https://itunes.apple.com/search?term=Allah%20Rakha%20Rahman&entity=song&limit=50&country=IN`;
    const response = await fetch(url);
    const data = await response.json();
    return data.results;
}

let arCurrentSongIdx = null;
let arIsPlaying = false;
let arSongsCache = [];

function renderArSongs(songs) {
    const list = document.getElementById('arSongList');
    if (!songs.length) {
    ...
    songs.forEach((song, idx) => {
        ...
        li.className = 'song-item' + (idx === arCurrentSongIdx && arIsPlaying ? ' active' : '');
        ...
        li.innerHTML = `
            <div class="play-on-hover">
                ...
                ${idx === arCurrentSongIdx && arIsPlaying ? frequencyBars : ''}
            </div>
            <div class="song-art">
...
function setupArVolumeControl() {
    const volumeSlider = document.getElementById('arVolumeSlider');
    const muteBtn = document.getElementById('arMuteBtn');
    const volumeIcon = document.getElementById('arVolumeIcon');
    let isMuted = false;
    let lastVolume = 1;
...
function playSongWithWaveform(song, idx) {
    ...
    arCurrentSongIdx = idx;
    arIsPlaying = true;
    renderArSongs(arSongsCache);
    ...
    wavesurfer.on('finish', () => { playPauseBtn.textContent = '▶'; arIsPlaying = false; renderArSongs(arSongsCache); });
    ...
    setupArVolumeControl();
}

fetchArSongs().then(songs => { arSongsCache = songs; renderArSongs(songs); }); 