// Fetch and render for SP Balasubrahmanyam songs from iTunes

async function fetchSpbSongs() {
    const url = `https://itunes.apple.com/search?term=SP+Balasubrahmanyam&entity=song&limit=12`;
    const response = await fetch(url);
    const data = await response.json();
    return data.results;
}

let spbCurrentSongIdx = null;
let spbIsPlaying = false;
let spbSongsCache = [];

function renderSpbSongs(songs) {
    const list = document.getElementById('spbSongList');
    if (!songs.length) {
    ...
    songs.forEach((song, idx) => {
        ...
        li.className = 'song-item' + (idx === spbCurrentSongIdx && spbIsPlaying ? ' active' : '');
        ...
        if (idx === spbCurrentSongIdx && spbIsPlaying) {
...
function setupSpbVolumeControl() {
    const volumeSlider = document.getElementById('spbVolumeSlider');
    const muteBtn = document.getElementById('spbMuteBtn');
    const volumeIcon = document.getElementById('spbVolumeIcon');
    let isMuted = false;
    let lastVolume = 1;
...
function playSongWithWaveform(song, idx) {
    ...
    spbCurrentSongIdx = idx;
    spbIsPlaying = true;
    renderSpbSongs(spbSongsCache);
    ...
    wavesurfer.on('finish', () => { playPauseBtn.textContent = '▶'; spbIsPlaying = false; renderSpbSongs(spbSongsCache); });
    ...
    setupSpbVolumeControl();
}

fetchSpbSongs().then(songs => { spbSongsCache = songs; renderSpbSongs(songs); }); 