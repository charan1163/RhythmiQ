// Fetch and render for Thaman songs from iTunes

async function fetchThamanSongs() {
    const url = `https://itunes.apple.com/search?term=SS%20Thaman&entity=song&limit=12`;
    const response = await fetch(url);
    const data = await response.json();
    return data.results;
}

let thamanCurrentSongIdx = null;
let thamanIsPlaying = false;
let thamanSongsCache = [];

function renderThamanSongs(songs) {
    const list = document.getElementById('thamanSongList');
    if (!songs.length) {
    ...
    songs.forEach((song, idx) => {
        ...
        li.className = 'song-item' + (idx === thamanCurrentSongIdx && thamanIsPlaying ? ' active' : '');
        ...
        if (idx === thamanCurrentSongIdx && thamanIsPlaying) {
...
function setupThamanVolumeControl() {
    const volumeSlider = document.getElementById('thamanVolumeSlider');
    const muteBtn = document.getElementById('thamanMuteBtn');
    const volumeIcon = document.getElementById('thamanVolumeIcon');
    let isMuted = false;
    let lastVolume = 1;
...
function playSongWithWaveform(song, idx) {
    ...
    thamanCurrentSongIdx = idx;
    thamanIsPlaying = true;
    renderThamanSongs(thamanSongsCache);
    ...
    wavesurfer.on('finish', () => { playPauseBtn.textContent = '▶'; thamanIsPlaying = false; renderThamanSongs(thamanSongsCache); });
    ...
    setupThamanVolumeControl();
}

fetchThamanSongs().then(songs => { thamanSongsCache = songs; renderThamanSongs(songs); }); 